...
/************************************************************************/
/*
 * Inpired by ASF, AcceptTradeOffer()
 */
 ...
 *    {"tradeid":"4749657481198302217"}
 * but, right now, I have no idea what to use it for so I'm ignoring
 * that. This is NOT that trade-id that was passed in!
 */

bool SteamBot::acceptTrade(SteamBot::TradeOfferID tradeOfferId)
{
    ...
    if (response->query->response.result()==boost::beast::http::status::ok)
    {
        ...
        BOOST_LOG_TRIVIAL(debug) << "acceptTrade response: " << string;
        return true;
    }
    return false;
}
